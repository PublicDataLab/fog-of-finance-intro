(function () {
    ...
    function animateStep(step, isEntering) {
        ...
        switch (step) {
            case 'step1':
                $svgLine = document.querySelector('line.step1');
                ...
                animateLine($svgLine, 'animationLine1', lineStart, lineEnd);
                if (isEntering) {
                    document.querySelector('text.step1').classList.add('shown');
                    $svgLine.classList.add('shown');
                } else {
                    document.querySelector('text.step1').classList.remove('shown');
            ...
            case 'step2':
                ...
                if (isEntering) {
                    document.querySelectorAll('path.step2, text.step2').forEach(function (el) { el.classList.add('shown') });
                    $svgLine.classList.add('shown');
                } else {
                    document.querySelectorAll('path.step2, text.step2').forEach(function (el) { el.classList.remove('shown') });
                    window.clearTimeout(timeout0);
                    timeout0 = window.setTimeout(function () {
                        $svgLine.classList.remove('shown');
                    }, 450);
            ...
            case 'step4':
                $svgLine = document.querySelector('line.step4');
                $oldAnimationEl = $svgLine.firstChild;
                if ($oldAnimationEl != null) {
                    $svgLine.removeChild($oldAnimationEl);
                }
                lineStart = isEntering ? $svgLine.getAttribute('y1') : $svgLine.getAttribute('y2');
                lineEnd = isEntering ? $svgLine.getAttribute('y2') : $svgLine.getAttribute('y1');
                animateLine($svgLine, 'animationLine4', lineStart, lineEnd);
                if (isEntering) {
                    document.querySelectorAll('path.step4').forEach(function (el) { el.classList.add('shown') });$svgLine.classList.add('shown');
                } else {
                    document.querySelectorAll('path.step4').forEach(function (el) { el.classList.remove('shown') });
                    window.clearTimeout(timeout0);
                    timeout0 = window.setTimeout(function () {
                        $svgLine.classList.remove('shown');
                    }, 450);
